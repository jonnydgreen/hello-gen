...
test("Utils", (t) => {
  ...
  t.test("readFile", (t) => {
    ...
    t.test("Should read files from input path", async (t) => {
      ...
      const utilFs: { readFile: typeof readFile } = t.mock(
        "../../src/util/util.fs",
        {
          fs: {
            promises: {
  ...
  t.test("writeFiles", (t) => {
    ...
    t.test("Should write files to output path", (t) => {
      ...
      const utilFs: { writeFiles: typeof writeFiles } = t.mock(
        "../../src/util/util.fs",
        {
          fs: {
            promises: {
    ...
    t.test("Should handle multiple errors", async (t) => {
      ...
      try {
        ...
        const expectedError = new FileWriteError(
          "Got 2 errors while writing file contents:",
          [
            {
              message: new Error("Error at path: errorPath1"),
              path: "errorPath1",
            },
            {
              message: new Error("Error at path: errorPath3"),
              path: "errorPath3",
            },
          ],
        );
