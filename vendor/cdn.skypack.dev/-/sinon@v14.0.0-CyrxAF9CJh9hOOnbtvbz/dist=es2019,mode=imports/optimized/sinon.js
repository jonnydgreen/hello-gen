...
(function() {
  ...
  var nextTick2 = require2("./util/core/next-tick");
  ...
  function prepareSandboxFromConfig(config2) {
    var sandbox = new Sandbox();
    if (config2.useFakeServer) {
      if (typeof config2.useFakeServer === "object") {
        sandbox.serverPrototype = config2.useFakeServer;
      }
      sandbox.useFakeServer();
    }
    if (config2.useFakeTimers) {
      if (typeof config2.useFakeTimers === "object") {
        sandbox.useFakeTimers(config2.useFakeTimers);
      } else {
  ...
  function exposeValue(sandbox, config2, key, value) {
    if (!value) {
      return;
    }
    if (config2.injectInto && !(key in config2.injectInto)) {
      config2.injectInto[key] = value;
  ...
  function createSandbox(config2) {
    ...
    var configuredSandbox = prepareSandboxFromConfig(config2);
    configuredSandbox.args = configuredSandbox.args || [];
    configuredSandbox.injectedKeys = [];
    configuredSandbox.injectInto = config2.injectInto;
    var exposed = configuredSandbox.inject({});
    if (config2.properties) {
      forEach(config2.properties, function(prop) {
        var value = exposed[prop] || prop === "sandbox" && configuredSandbox;
        exposeValue(configuredSandbox, config2, prop, value);
      });
    } else {
      exposeValue(configuredSandbox, config2, "sandbox");
    }
...
${wrappedMethodDesc.stackTraceError.stack}`;
  ...
  exports.useFakeTimers = function(dateOrConfig) {
    ...
    if (argumentIsObject) {
      var config2 = extend.nonEnum({}, dateOrConfig);
      var globalCtx = config2.global;
      delete config2.global;
      return createClock(config2, globalCtx);
    }
...
To automatically clean-up native timers, use \`shouldClearNativeTimers\`.`);
  ...
  function configureLogger(config2) {
    config2 = config2 || {};
    if (!config2.hasOwnProperty("logger")) {
      config2.logger = function() {
      };
    }
    if (!config2.hasOwnProperty("useImmediateExceptions")) {
      config2.useImmediateExceptions = true;
    }
    if (!config2.hasOwnProperty("setTimeout")) {
      config2.setTimeout = realSetTimeout;
    }
    return function logError(label, e) {
      ...
      config2.logger(`${msg}[${err.name}] ${err.message}`);
      if (err.stack) {
        config2.logger(err.stack);
      }
      if (config2.useImmediateExceptions) {
        throwLoggedError();
      } else {
        config2.setTimeout(throwLoggedError, 0);
      }
    };
  }
  ...
  var fakeServer = {
    ...
    configure: function(config2) {
      ...
      config2 = config2 || {};
      Object.keys(config2).forEach(function(setting) {
        if (setting in allowlist) {
          self2[setting] = config2[setting];
        }
      });
      self2.logError = configureLogError(config2);
    },
    ...
    respond: function respond() {
      ...
      var requests = queue2.splice(0, queue2.length);
      var self2 = this;
...
`;
  ...
  function fakeXMLHttpRequestFor(globalScope) {
    ...
    function FakeXMLHttpRequest(config2) {
      ...
      this.responseType = "";
      this.response = "";
      this.logError = configureLogError(config2);
...
`;
  ...
  (function(global2, factory) {
    ...
    var globalObject = typeof self === "object" ? self : global$1;
...
export default sinon$1;
export {_FakeXMLHttpRequest as FakeXMLHttpRequest, _addBehavior as addBehavior, _assert as assert, _createFakeServer as createFakeServer, _createFakeServerWithClock as createFakeServerWithClock, _createSandbox as createSandbox, _createStubInstance as createStubInstance, _defaultConfig as defaultConfig, _expectation as expectation, _fake as fake, _fakeServer as fakeServer, _fakeServerWithClock as fakeServerWithClock, _getFakes as getFakes, _getRestorers as getRestorers, _inject as inject, _leakThreshold as leakThreshold, _match as match, _mock as mock, _promise as promise, _replace as replace, _replaceGetter as replaceGetter, _replaceSetter as replaceSetter, _reset as reset, _resetBehavior as resetBehavior, _resetHistory as resetHistory, _restore as restore, _restoreContext as restoreContext, _restoreObject as restoreObject, _serverPrototype as serverPrototype, _setFormatter as setFormatter, _spy as spy, _stub as stub, _timers as timers, _useFakeServer as useFakeServer, _useFakeTimers as useFakeTimers, _useFakeXMLHttpRequest as useFakeXMLHttpRequest, _usingPromise as usingPromise, _verify as verify, _verifyAndRestore as verifyAndRestore, _xhr as xhr};
