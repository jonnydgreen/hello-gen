...
function ordinaryGetMetadataKeys(
  target: Target,
  propertyKey?: PropertyKey
): MetadataKey[] {
  return ordinaryGetOwnMetadataKeys(target, propertyKey).length
    ? ordinaryGetOwnMetadataKeys(target, propertyKey)
    : Object.getPrototypeOf(target)
    ? ordinaryGetOwnMetadataKeys(Object.getPrototypeOf(target), propertyKey)
    : [];
}

function ordinaryGetOwnMetadataKeys(
  target: Target,
  propertyKey?: PropertyKey
): MetadataKey[] {
  if (target === undefined) {
  ...
  return typeof metadataMap === "undefined"
    ? []
    : Array.from(metadataMap.keys());
}
...
export function getMetadataKeys(
  ...
  return ordinaryGetMetadataKeys(target, propertyKey);
}
...
export function getOwnMetadataKeys(
  ...
  return ordinaryGetOwnMetadataKeys(target, propertyKey);
}
